...
public class ClientHandledScreen extends GenericContainerScreen implements OldEventBehavior, IgnoreCloseScreenPacket {
	...
	protected final int getHandledScreenX() {
		return ((HandledScreenAccessor) this).getX();
	}
	protected final void setHandledScreenX(int x) {
		((HandledScreenAccessor) this).setX(x);
	}
	protected final int getHandledScreenY() {
		return ((HandledScreenAccessor) this).getY();
	}
	protected final int getHandledBackgroundWidth() {
		return ((HandledScreenAccessor) this).getBackgroundWidth();
	}
	protected final int getHandledTitleX() {
		return ((HandledScreenAccessor) this).getTitleX();
	}
	protected final int getHandledTitleY() {
		return ((HandledScreenAccessor) this).getTitleY();
	}
	protected final Text getHandledPlayerInventoryTitle() {
		return ((HandledScreenAccessor) this).getPlayerInventoryTitle();
	}
	protected final int getHandledPlayerInventoryTitleX() {
		return ((HandledScreenAccessor) this).getPlayerInventoryTitleX();
	}
	protected final int getHandledPlayerInventoryTitleY() {
		return ((HandledScreenAccessor) this).getPlayerInventoryTitleY();
	}
	protected final Slot getHandledFocusedSlot() {
		return ((HandledScreenAccessor) this).getFocusedSlot();
	}
	...
	protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
		GenericContainerScreenHandler handler = getScreenHandler();
		int x = getHandledScreenX();
		int y = getHandledScreenY();
		int backgroundWidth = getHandledBackgroundWidth();
	...
	protected void drawForeground(MatrixStack matrices, int mouseX, int mouseY) {
		...
		MVDrawableHelper.drawTextWithoutShadow(matrices, MainUtil.client.textRenderer, getRenderedTitle(), getHandledTitleX(), getHandledTitleY(), 4210752);
		MVDrawableHelper.drawTextWithoutShadow(matrices, MainUtil.client.textRenderer, getHandledPlayerInventoryTitle(),
				getHandledPlayerInventoryTitleX(), getHandledPlayerInventoryTitleY(), 4210752);
	}
	@Override
	...
	protected void onMouseClick(Slot slot, int slotId, int button, SlotActionType actionType) {
		...
		if (slot != null) {
			...
			if (lockedSlotsInfo.isBlocked(slot, button, actionType, false)) {
				if (lockedSlotsInfo.isCopyLockedItem() && slot.inventory != MainUtil.client.player.getInventory()) {
					switch (actionType) {
						case PICKUP, PICKUP_ALL -> {
							...
							if (!handler.getCursorStack().isEmpty() &&
									!ItemStack.canCombine(item, handler.getCursorStack())) {
