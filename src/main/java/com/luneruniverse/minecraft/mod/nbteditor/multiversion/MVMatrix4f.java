...
public class MVMatrix4f {
	...
	private static final Supplier<Reflection.MethodInvoker> MatrixStack_Entry_getPositionMatrix =
			Reflection.getOptionalMethod(MatrixStack.Entry.class, "method_23761", MethodType.methodType(Matrix4f_class));
	public static MVMatrix4f getPositionMatrix(MatrixStack.Entry matrix) {
		return new MVMatrix4f(Version.<Object>newSwitch()
				.range("1.19.3", null, () -> matrix.getPositionMatrix())
				.range(null, "1.19.2", () -> MatrixStack_Entry_getPositionMatrix.get().invoke(matrix))
				.get());
	}
	...
	private static final Supplier<Reflection.MethodInvoker> VertexConsumer_vertex =
			Reflection.getOptionalMethod(VertexConsumer.class, "method_22918", MethodType.methodType(VertexConsumer.class, Matrix4f_class, float.class, float.class, float.class));
	public VertexConsumer applyToVertex(VertexConsumer buffer, float x, float y, float z) {
		return Version.<VertexConsumer>newSwitch()
				.range("1.19.3", null, () -> buffer.vertex((Matrix4f) value, x, y, z))
				.range(null, "1.19.2", () -> VertexConsumer_vertex.get().invoke(buffer, value, x, y, z))
				.get();
	}
