...
public class MVDrawableHelper {
	...
	public static void super_render(Class<?> callerClass, Drawable caller, MatrixStack matrices, int mouseX, int mouseY, float delta) {
		try {
			...
			if (Version.<Boolean>newSwitch()
				...
				methodName = Reflection.getMethodName(Drawable.class, "method_25394",
						MethodType.methodType(void.class, matrixType, int.class, int.class, float.class));
			}
	...
	public static void fill(MatrixStack matrices, int x1, int y1, int x2, int y2, int color) {
		Version.newSwitch()
				.range("1.20.0", null, () -> getDrawContext(matrices).fill(x1, y1, x2, y2, color))
				.range(null, "1.19.4", () -> call("method_25294", void.class, new Class<?>[] {int.class, int.class, int.class, int.class, int.class}, matrices, x1, y1, x2, y2, color))
				.run();
	}
	...
	public static void drawTextWithShadow(MatrixStack matrices, TextRenderer textRenderer, Text text, int x, int y, int color) {
		Version.newSwitch()
				.range("1.20.0", null, () -> getDrawContext(matrices).drawTextWithShadow(textRenderer, text, x, y, color))
				.range(null, "1.19.4", () -> call("method_27535", void.class, new Class<?>[] {TextRenderer.class, Text.class, int.class, int.class, int.class}, matrices, textRenderer, text, x, y, color))
				.run();
	}
	...
	public static void drawCenteredTextWithShadow(MatrixStack matrices, TextRenderer textRenderer, Text text, int x, int y, int color) {
		Version.newSwitch()
				.range("1.20.0", null, () -> getDrawContext(matrices).drawCenteredTextWithShadow(textRenderer, text, x, y, color))
				.range(null, "1.19.4", () -> call("method_27534", void.class, new Class<?>[] {TextRenderer.class, Text.class, int.class, int.class, int.class}, matrices, textRenderer, text, x, y, color))
				.run();
	}
	...
	public static void drawTexture(MatrixStack matrices, Identifier texture, int x, int y, float u, float v, int width, int height, int textureWidth, int textureHeight) {
		Version.newSwitch()
				...
				.range(null, "1.19.4", () -> DrawContext_drawTexture.get().invoke(getDrawContext(matrices), texture, x, y, u, v, width, height, textureWidth, textureHeight))
				.run();
	}
	...
	public static void renderItem(MatrixStack matrices, float zOffset, boolean setScreenZOffset, ItemStack item, int x, int y) {
		...
		context.drawItemInSlot(MainUtil.client.textRenderer, item, x, y);
	}
