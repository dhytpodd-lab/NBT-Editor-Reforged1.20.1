...
public class Version {
	...
	private static final String FALLBACK_RELEASE_TARGET = "1.20.1";
	private static final int FALLBACK_DATA_VERSION = 3465;
	...
	private static void readVersionJson() {
		InputStream in = Version.class.getResourceAsStream("/version.json");
		if (in == null) {
			releaseTarget = FALLBACK_RELEASE_TARGET;
			dataVersion = FALLBACK_DATA_VERSION;
			return;
		}
		try (in; InputStreamReader reader = new InputStreamReader(in)) {
	...
	private static void readDataVersionsJson() {
		InputStream in = Version.class.getResourceAsStream("/assets/nbteditor/data_versions.json");
		if (in == null) {
			dataVersions = Map.of(FALLBACK_RELEASE_TARGET, FALLBACK_DATA_VERSION);
			mcVersions = Map.of(FALLBACK_DATA_VERSION, FALLBACK_RELEASE_TARGET);
			return;
		}
		try (in) {
