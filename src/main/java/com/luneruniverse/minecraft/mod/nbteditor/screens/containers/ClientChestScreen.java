...
public class ClientChestScreen extends ClientHandledScreen {
	...
	private void setPageData(ClientChestPage pageData) {
		...
		for (int i = 0; i < items.length; i++)
			getScreenHandler().getSlot(i).setStackNoCallbacks(items[i] == null ? ItemStack.EMPTY : items[i].copy());
	...
	protected void init() {
		this.clearChildren();
		super.init();
		setHandledScreenX(getHandledScreenX() + 87 / 2);
		int x = getHandledScreenX();
		int y = getHandledScreenY();
		
		nameField = new NamedTextFieldWidget(x - 87, y, 83, 16) {
			@Override
		...
		pageField = new TextFieldWidget(MainUtil.client.textRenderer, x - 63, y + 22, 35, 16, TextInst.of("")) {
			@Override
			public boolean mouseClicked(double mouseX, double mouseY, int button) {
				boolean output = super.mouseClicked(mouseX, mouseY, button);
				if (output)
		...
		this.addDrawableChild(prevPage = MVMisc.newButton(x - 87, y + 20, 20, 20, TextInst.of("<"), btn -> {
			navigationClicked = true;
			prevPage();
		...
		this.addDrawableChild(nextPage = MVMisc.newButton(x - 24, y + 20, 20, 20, TextInst.of(">"), btn -> {
			navigationClicked = true;
			nextPage();
		}, ConfigScreen.isKeybindsHidden() ? null : new MVTooltip(TextInst.literal("")
				.append(nextKeybind).append(TextInst.translatable("nbteditor.keybind.page.next")))));
		
		this.addDrawableChild(prevPageJump = MVMisc.newButton(x - 87, y + 44, 39, 20, TextInst.of("<<"), btn -> {
			navigationClicked = true;
			prevPageJump();
		...
		this.addDrawableChild(nextPageJump = MVMisc.newButton(x - 43, y + 44, 39, 20, TextInst.of(">>"), btn -> {
			navigationClicked = true;
			nextPageJump();
		}, ConfigScreen.isKeybindsHidden() ? null : new MVTooltip(TextInst.translatable("nbteditor.keybind.page.shift")
				.append(nextKeybind).append(TextInst.translatable("nbteditor.keybind.page.next_jump")))));
		
		ButtonWidget lockBtn = this.addDrawableChild(MVMisc.newButton(x - 87, y + 68, 83, 20, ConfigScreen.isLockSlots() ? TextInst.translatable("nbteditor.client_chest.slots.unlock") : TextInst.translatable("nbteditor.client_chest.slots.lock"), btn -> {
			navigationClicked = true;
			if (ConfigScreen.isLockSlotsRequired()) {
				MVMisc.setWidgetActive(btn, false);
		...
		MVMisc.setWidgetActive(lockBtn, !ConfigScreen.isLockSlotsRequired());
		
		this.addDrawableChild(MVMisc.newButton(x - 87, y + 92, 83, 20, TextInst.translatable("nbteditor.client_chest.reload_page"), btn -> {
			navigationClicked = true;
			LoadingScreen.show(ClientChestHelper.reloadPage(PAGE), this::close, (loaded, pageData) -> show());
		}));
		
		this.addDrawableChild(MVMisc.newButton(x - 87, y + 116, 83, 20, TextInst.translatable("nbteditor.client_chest.clear_page"), btn -> {
			navigationClicked = true;
			MainUtil.client.setScreen(new FancyConfirmScreen(value -> {
				if (value) {
					getScreenHandler().getInventory().clear();
					dynamicItems = new DynamicItems();
					save();
				}
				
				MainUtil.client.setScreen(ClientChestScreen.this);
	...
	public void updatePageNavigation() {
		...
		MVMisc.setWidgetActive(prevPage, PAGE != 0);
		MVMisc.setWidgetActive(nextPage, PAGE != maxPage);
		MVMisc.setWidgetActive(prevPageJump, prevPageJumpTarget != -1);
		MVMisc.setWidgetActive(nextPageJump, nextPageJumpTarget != -1);
	}
	...
	public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
		...
		Slot focusedSlot = getHandledFocusedSlot();
		if (focusedSlot != null) {
			boolean lockedSlot = (focusedSlot.inventory == getScreenHandler().getInventory() &&
	...
	private void save() {
		...
		for (int i = 0; i < getScreenHandler().getInventory().size(); i++)
			items[i] = getScreenHandler().getInventory().getStack(i).copy();
	...
	protected Text getRenderedTitle() {
		EditableText title = TextInst.copy(getTitle()).append(" (" + (PAGE + 1) + ")");
	...
	private void prevPage() {
		if (!MVMisc.isWidgetActive(prevPage))
			return;
		PAGE--;
		show();
	}
	private void nextPage() {
		if (!MVMisc.isWidgetActive(nextPage))
			return;
		PAGE++;
		show();
	}
	...
	private void prevPageJump() {
		if (!MVMisc.isWidgetActive(prevPageJump))
			return;
		PAGE = prevPageJumpTarget;
		show();
	}
	private void nextPageJump() {
		if (!MVMisc.isWidgetActive(nextPageJump))
			return;
