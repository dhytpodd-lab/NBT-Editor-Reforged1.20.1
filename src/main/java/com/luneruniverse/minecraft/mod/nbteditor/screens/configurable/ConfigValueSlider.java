...
public class ConfigValueSlider<T extends Number> extends SliderWidget implements ConfigValue<T, ConfigValueSlider<T>> {
	...
	protected void applyValue() {
		actualValue = caster.apply((int) (MVMisc.getSliderValue(this) * (max.doubleValue() - min.doubleValue()) / step.doubleValue()) * step.doubleValue() + min.doubleValue());
	...
	public void setValue(T value) {
		MVMisc.setSliderValue(this, (value.doubleValue() - min.doubleValue()) / (max.doubleValue() - min.doubleValue()));
		this.actualValue = value;
		onChanged.forEach(listener -> listener.onValueChanged(this));
		updateMessage();
	}
	...
	public ConfigValueSlider<T> clone(boolean defaults) {
		return new ConfigValueSlider<T>(MVMisc.getWidgetWidth(this), defaults ? defaultValue : actualValue, defaultValue, min, max, step, msg, caster, onChanged);
	}
