...
public class TextAreaScreen extends OverlaySupportingScreen {
	...
	protected void init() {
		...
		this.addDrawableChild(MVMisc.newButton(MVMisc.getWidgetX(done) + done.getWidth() + 10, 20,
				done.getWidth(), 20, ScreenTexts.CANCEL, btn -> close()));
		...
		textArea = addDrawableChild(MultiLineTextFieldWidget.create(textArea, 20, 50, width - 40, height - 70, text, formatter == null ? null : str -> {
			...
			MVMisc.setWidgetActive(done, formattedText.isSuccess());
