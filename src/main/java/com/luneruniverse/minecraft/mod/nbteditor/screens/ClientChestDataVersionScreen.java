...
public class ClientChestDataVersionScreen extends TickableSupportingScreen {
	...
	protected void init() {
		...
		ButtonWidget prevPageBtn = addDrawableChild(MVMisc.newButton(dontUpdatePageX + 56, height / 2 - 34, 20, 20, TextInst.of("<"), btn -> prevPage(),
		...
		MVMisc.setWidgetActive(prevPageBtn, ClientChestScreen.PAGE > 0);
		ButtonWidget nextPageBtn = addDrawableChild(MVMisc.newButton(dontUpdatePageX + 80, height / 2 - 34, 20, 20, TextInst.of(">"), btn -> nextPage(),
				ConfigScreen.isKeybindsHidden() ? null : new MVTooltip(TextInst.literal("")
						.append(nextKeybind).append(TextInst.translatable("nbteditor.keybind.page.next")))));
		MVMisc.setWidgetActive(nextPageBtn, ClientChestScreen.PAGE < NBTEditorClient.CLIENT_CHEST.getPageCount() - 1);
		...
		ButtonWidget importPageBtn = addDrawableChild(MVMisc.newButton(width / 2 - 158, height / 2 - 10, 100, 20,
		...
		MVMisc.setWidgetActive(importPageBtn, dataVersionStatus == DataVersionStatus.UNKNOWN);
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		if (fullButtons) {
			...
			MVMisc.setWidgetActive(updatePageBtn,
					dataVersionStatus == DataVersionStatus.UNKNOWN ? dataVersion.isValid() : dataVersion.getText().isEmpty());
		}
