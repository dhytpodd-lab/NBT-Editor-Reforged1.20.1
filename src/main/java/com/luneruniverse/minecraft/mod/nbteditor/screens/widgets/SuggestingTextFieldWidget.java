...
import com.luneruniverse.minecraft.mod.nbteditor.mixin.ChatInputSuggestorAccessor;
import com.luneruniverse.minecraft.mod.nbteditor.mixin.ChatInputSuggestorSuggestionWindowAccessor;
...
public class SuggestingTextFieldWidget extends NamedTextFieldWidget {
	...
	public SuggestingTextFieldWidget(Screen screen, int x, int y, int width, int height, TextFieldWidget copyFrom) {
		...
		suggestor = new ChatInputSuggestor(MainUtil.client, screen, this, MainUtil.client.textRenderer, false, true, 0, 7, false, 0x80000000) {
			@Override
			public void refresh() {
				ChatInputSuggestorAccessor accessor = (ChatInputSuggestorAccessor) this;
				if (!accessor.nbteditor$isCompletingSuggestions()) {
					SuggestingTextFieldWidget.this.setSuggestion(null);
					clearWindow();
				}
				accessor.nbteditor$getMessages().clear();
				if (accessor.nbteditor$getWindow() == null || !accessor.nbteditor$isCompletingSuggestions()) {
					CompletableFuture<Suggestions> pendingSuggestions;
					if (suggestions == null) {
						pendingSuggestions = new SuggestionsBuilder("", 0).buildFuture();
					} else {
						pendingSuggestions = suggestions.apply(SuggestingTextFieldWidget.this.getText(), SuggestingTextFieldWidget.this.getCursor());
					}
					accessor.nbteditor$setPendingSuggestions(pendingSuggestions);
					pendingSuggestions.thenRun(() -> {
						if (!pendingSuggestions.isDone() || accessor.nbteditor$getPendingSuggestions() != pendingSuggestions)
							return;
						accessor.nbteditor$showCommandSuggestions();
					});
				}
			}
			@Override
		...
		((ChatInputSuggestorAccessor) suggestor).nbteditor$setParse(new ParseResults<>(null));
	...
	protected boolean shouldShowName() {
		return ((ChatInputSuggestorAccessor) suggestor).nbteditor$getWindow() == null;
	}
	...
	public boolean isMouseOver(double mouseX, double mouseY) {
		ChatInputSuggestor.SuggestionWindow window = ((ChatInputSuggestorAccessor) suggestor).nbteditor$getWindow();
		if (window != null) {
			if (((ChatInputSuggestorSuggestionWindowAccessor) window).nbteditor$getArea().contains((int) mouseX, (int) mouseY))
				return true;
		}
