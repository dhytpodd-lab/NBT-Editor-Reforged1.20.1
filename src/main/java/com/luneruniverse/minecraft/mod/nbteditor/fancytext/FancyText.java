...
public class FancyText {
	...
	public static Map.Entry<String, Boolean> stringify(Text text, Style base) {
		...
		Identifier baseFont = base.getFont();
		...
		AtomicReference<Identifier> font = new AtomicReference<>(baseFont);
		...
		text.visit((partStyle, partText) -> {
			if (!Objects.equals(partStyle.getClickEvent(), clickEvent.getPlain()) ||
					...
					!Objects.equals(StyleUtil.getFontRaw(partStyle), font.getPlain())) {
				...
				if (insertion.getPlain() != null)
					output.append(')');
				if (!Objects.equals(font.getPlain(), baseFont))
					output.append(')');
				...
				font.setPlain(StyleUtil.getFontRaw(partStyle));
				...
				if (!Objects.equals(StyleUtil.getFontRaw(partStyle), baseFont)) {
					output.append("[font]{");
					output.append(StyleUtil.getFontRaw(partStyle).toString());
					output.append("}(");
				}
			}
			...
			Style previousStyle = currentStyle.getPlain();
			Style changes = StyleUtil.minus(partStyle, previousStyle);
			currentStyle.setPlain(partStyle);
			
			if (!partStyle.isBold() && previousStyle.isBold() ||
					!partStyle.isItalic() && previousStyle.isItalic() ||
					!partStyle.isUnderlined() && previousStyle.isUnderlined() ||
					!partStyle.isStrikethrough() && previousStyle.isStrikethrough() ||
					!partStyle.isObfuscated() && previousStyle.isObfuscated()) {
				output.append("&r");
