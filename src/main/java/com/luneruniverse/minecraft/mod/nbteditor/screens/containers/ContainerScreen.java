...
public class ContainerScreen<L extends LocalNBT> extends ClientHandledScreen {
	...
	private ContainerScreen(NBTReference<L> ref) {
		...
		this.unsavedTitle = TextInst.copy(getTitle()).append("*");
		...
		for (int i = 0; i < contents.length; i++)
			getScreenHandler().getSlot(i).setStackNoCallbacks(contents[i] == null ? ItemStack.EMPTY : contents[i].copy());
	}
	...
	protected void init() {
		...
		if (ref instanceof ItemReference item && item.isLockable()) {
			ButtonWidget lockBtn = this.addDrawableChild(MVMisc.newButton(16, 64, 83, 20, ConfigScreen.isLockSlots() ? TextInst.translatable("nbteditor.client_chest.slots.unlock") : TextInst.translatable("nbteditor.client_chest.slots.lock"), btn -> {
		...
		addDrawableChild(MVMisc.newTexturedButton(MainUtil.client.getWindow().getScaledWidth() - 36, 22, 20, 20, 20,
				LocalFactoryScreen.FACTORY_ICON,
				btn -> MainUtil.client.setScreen(new LocalFactoryScreen<>(ref)),
				new MVTooltip("nbteditor.factory")));
	}
	
	@Override
	protected Text getRenderedTitle() {
		return saved ? getTitle() : unsavedTitle;
	}
	...
	private void save() {
		ItemStack[] contents = new ItemStack[getScreenHandler().getInventory().size()];
		for (int i = 0; i < contents.length; i++)
			contents[i] = getScreenHandler().getInventory().getStack(i);
	...
	public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
		...
		if (focusedSlot != null && (focusedSlot.id < numSlots || focusedSlot.inventory != getScreenHandler().getInventory())) {
	...
	public void removed() {
		for (int i = numSlots; i < 27; i++) { // Items that will get deleted
			ItemStack item = getScreenHandler().getInventory().getStack(i);
