...
import com.luneruniverse.minecraft.mod.nbteditor.mixin.CreativeInventoryScreenCreativeSlotAccessor;
...
public class MixinLink {
	...
	private static Slot unwrapCreativeSlot(Slot slot) {
		if (slot instanceof CreativeInventoryScreenCreativeSlotAccessor creativeSlot)
			return creativeSlot.getSlot();
		return slot;
	}
	...
	public static void keyPressed(HandledScreen<?> source, int keyCode, int scanCode, int modifiers, CallbackInfoReturnable<Boolean> info) {
		...
		Slot hoveredSlot = unwrapCreativeSlot(((HandledScreenAccessor) source).getFocusedSlot());
